...
namespace NinetiesTV
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Print("All the genres from the 80s", AllGenresFromThe80s(shows));
            Print("A list of all distint genres", AllGenres(shows));
            Print("When each show started", WhenEachShowStarted(shows));
        }
        ...
        **************************************************************************************************/
        ...
        static string AllGenresFromThe80s(List<Show> shows)
        {
            return string.Join(", ", shows.Where(s => s.StartYear >= 1980 && s.StartYear <= 1989).SelectMany(s => s.Genres).Distinct());
        }
        // 2. Print a unique list of genres.
        static string AllGenres(List<Show> shows)
        {
        ...
        static List<Show> WhenEachShowStarted(List<Show> shows)
        {
            return shows.OrderBy(s => s.StartYear).GroupBy(s => s.StartYear).SelectMany(s => s).ToList();
        }
